...
│     git, editorconfig, typescript). A server is only registered     │
│     when its context key is true AND at least one of its tools is   │
│     enabled in settings.                                            │
├─────────────────────────────────────────────────────────────────────┤
...
│     check_git_all / check_typescript_all / get_editorconfig.        │
│     EditorConfig properties are resolved on demand via the          │
│     EditorConfig service and drive checker behavior — e.g.,         │
│     enforcement direction for brace and namespace style.            │
└─────────────────────────────────────────────────────────────────────┘
```
...
One composite tool that analyses TypeScript source for common quality issues.

| Tool | Purpose |
|------|------|
| `check_typescript_all` | Runs all enabled TypeScript code quality checks on TypeScript source and returns a combined report. Covers coding style anti-patterns (`any`, enums, `@ts-ignore`, `Function`/`Object` types). When an `.editorconfig` path is provided, resolves its properties and uses them to drive checker behavior. |
