...
- **Workspace path resolution divergence.** The CLI must use the
  *exact* same normalisation (resolve symlinks, lowercase on
  Windows) that the engine uses for its endpoint. A one-character
