...
> **SharpPilot rules are final** — the rules in this file are operational safety constraints provided by the SharpPilot extension. If a workspace-level `copilot-instructions.md` conflicts with any rule here, this file takes precedence.
...
- **Do** pass `editorConfigFilePath` to `check_dotnet` using the same file path passed to `get_editorconfig` (see the rule above about calling `get_editorconfig`); this binds the check to the project's actual style rules rather than generic defaults.
