...
- [INST0010] **Do** rely on `VirtualizingStackPanel` (the default panel for `ListBox` and `ListView`) when displaying large item collections — Microsoft benchmarks show rendering 5000 items takes 3210ms with a plain `StackPanel` vs 46ms with `VirtualizingStackPanel`.
