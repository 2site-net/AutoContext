...
/// no entry catalogues (a stray, exempting the always-attached files the catalog
...
/// and must not also be catalogued. Authoring slips (a malformed document, a blank
...
internal sealed class InstructionsCatalogReader : IInstructionsCatalogReader
{
        ...
        IReadOnlyDictionary<string, CorpusFileParsedResult> corpus)
    {
        ...
        var cataloguedFiles = ValidateEntries(catalog.Instructions, categories);
        ValidateAlwaysAttached(catalog.AlwaysAttached, cataloguedFiles);
        ...
        IReadOnlyDictionary<string, CorpusFileParsedResult> corpus)
    {
        ...
        var cataloguedFiles = new HashSet<string>(StringComparer.Ordinal);

        foreach (var entry in entries)
        {
            cataloguedFiles.Add(entry.FileName);
        ...
        foreach (var file in corpus.Values)
        {
            ...
            if (!cataloguedFiles.Contains(file.FileName))
            {
                throw Fail("corpus file '" + file.FileName + "' is not catalogued");
            }
        }
    }
    ...
    private static void ValidateAlwaysAttached(
        IReadOnlyList<string> alwaysAttached,
        HashSet<string> cataloguedFiles)
    {
        ...
        foreach (var fileName in alwaysAttached)
        {
            ...
            if (cataloguedFiles.Contains(fileName))
            {
                throw Fail("file '" + fileName + "' is declared always-attached and also catalogued");
            }
        }
    }
