...
/// <summary>
/// The <c>Instructions.*</c> read handler. It reads the immutable
/// manifest snapshot and the workspace config through their accessor
/// seams, projects listing rows through
/// <see cref="InstructionsListProjector"/>, and projects body text
/// through <see cref="InstructionsBodyProjector"/> or
...
/// failures reply <see cref="JsonRpcErrorCodes.InternalError"/>; in every
/// case the connection keeps serving.
/// </summary>
internal sealed partial class InstructionsRpcHandler : IRpcMethodHandler
{
    ...
    private readonly InstructionsFrameStream _instructionsFrameStream = new();
    ...
    private readonly ILogger<InstructionsRpcHandler> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="InstructionsRpcHandler"/>
    /// class.
    /// </summary>
    public InstructionsRpcHandler(
        IInstructionsManifestAccessor instructionsManifestAccessor,
        InstructionsListProjector instructionsListProjector,
        ...
        ILogger<InstructionsRpcHandler> logger)
    {
        ArgumentNullException.ThrowIfNull(instructionsManifestAccessor);
        ArgumentNullException.ThrowIfNull(instructionsListProjector);
        ...
        _instructionsListProjector = instructionsListProjector;
        ...
        JsonRpcRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            InstructionsMethods.Categories => HandleInstructionsCategories(),
            InstructionsMethods.Get => await HandleInstructionsGetAsync(request, cancellationToken).ConfigureAwait(false),
            InstructionsMethods.GetAll => await HandleInstructionsGetAllAsync(cancellationToken).ConfigureAwait(false),
