...
describe('PipeStreamingClient', () => {
    it('writes serialized items over the pipe', async () => {
        ...
        try {
            client.post('hello');
            client.post('world');
            await waitFor(() => Buffer.concat(echo.captured).toString('utf8') === 'helloworld');
        }
        finally {
    ...
    it('writes the greeting before any items', async () => {
        ...
        try {
            client.post('A');
            await waitFor(() => Buffer.concat(echo.captured).toString('utf8') === 'GREET\nA');
        }
        finally {
            await client.dispose();
            await echo.stop();
    ...
    it('routes items to the fallback when the pipe name is empty', async () => {
        ...
        try {
            client.post('a');
            client.post('b');
            await waitFor(() => fallback.length === 2);
