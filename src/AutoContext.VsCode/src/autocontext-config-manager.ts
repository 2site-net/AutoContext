...
export class AutoContextConfigManager implements vscode.Disposable {
    ...
    private readonly fileManager: AutoContextFileManager;
    ...
    constructor(
        ...
        fileManager?: AutoContextFileManager,
    ) {
        this.fileManager = fileManager ?? new AutoContextFileManager(logger);
    ...
    async read(): Promise<AutoContextConfig> {
        ...
        try {
            this.cachedConfig = await this.fileManager.load(path);
            return this.cachedConfig;
        } catch (err) {
            const isNotFound = AutoContextFileManager.isNotFound(err);
    ...
    private async writeConfig(config: AutoContextConfig): Promise<void> {
        ...
        await this.fileManager.save(path, ordered);
