...
export class InstructionsCatalog {
    ...
    constructor(data: readonly InstructionsFileEntry[], metadata?: ReadonlyMap<string, InstructionsMetadataEntry>) {
        this.entries = data.map(d => new InstructionsCatalogEntry(d, metadata?.get(d.fileName)));
